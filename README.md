...
- [`mcp-server/`](./mcp-server) — MCP adapter wrapping the HTTP API as agent tools for Claude Desktop, Claude Code, Codex CLI, OpenClaw, etc. See [`mcp-server/README.md`](./mcp-server/README.md) for install commands and [`mcp-server/SKILL.md`](./mcp-server/SKILL.md) for when an agent should reach for it. The same package also ships `printer-mcp-guestbook`, a one-tool server that lets strangers on a public chat host send you a printed note and do nothing else; see the guestbook section of that README.
